...
const hideDistance = computed(() => {
    if (!message.value) return 180; // Default fallback
    const rect = message.value.getBoundingClientRect();
    return rect.height + (screenStore.isMobile ? 180 : 120); // Height of message + buffer to ensure it's fully hidden
});

onMounted(() => {

    const animatedPart = message.value.querySelector('svg path:nth-child(2)');
    if (animatedPart) {
        const originalY = 0
        gsap.to(animatedPart, {
            y: screenStore.isMobile ? originalY - 60 : originalY + 30,
            duration: 1,
            ease: 'easeOut',
            onComplete: () => { animatedPart.position.y = originalY },
            repeat: -1
        })
    }
...
watch(progress, (newValue, oldValue) => {
    ...
    // Determine direction
    const isForward = newValue > oldValue;
    const shouldShow = newValue < 80; // Show when progress < 100
    const shouldHide = newValue >= 80; // Hide when progress >= 100

    if ((isForward && shouldHide) || (!isForward && shouldShow)) {
        // Kill any existing animation on the message element
        gsap.killTweensOf(message.value);

        // Animate with dynamic distance and 3D effect
        gsap.to(message.value, {
            y: isForward ? hideDistance.value : 0, // Use computed distance
            scale: isForward ? 2 : 1, // Grow when hiding, shrink back when showing
            duration: 0.375,
            ease: 'easeIn',
            onStart: () => { animationActive = true; },
...
<template>
    ...
    <div class="wrap fixed inset-x-0 bottom-0 z-10 pb-28">

        <div ref="message" class="message flex flex-col items-center text-white text-lg md:text-sm drop-shadow-white">
            ...
            <svg v-else class="mouse w-6 mb-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512">
            ...
            <div v-if="screenStore.isMobile">SWIPE UP TO CONTINUE</div>
            <div v-else>SCROLL TO CONTINUE</div>
